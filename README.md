...
When you open one of these sub-patches it should look like the one shown in _figure 7_. The content of these patches was based on the sampler patch created by cheetomoskeeto in a YouTube video, which can be found here: https://www.youtube.com/watch?v=boX0v54SqtU. If you follow the link you will find a full tutorial an explanation for each individual element within this sub-patch.  

_Figure 7: A print-screen image showing how the sampler in each sub-patch should be set-up. The only feature that should differ between each sub-patch is the sample that is used and therefore the playback speed of that sample as well_
